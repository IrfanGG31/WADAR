...
- **3 bug nyata ketemu & diperbaiki lewat testing manual** (tidak akan ketangkep lint/typecheck/unit test saja): (1) BullMQ melarang `:` di nama queue — `consumerQueueName` diganti pakai `.`; (2) `outbox-relay.tick()` crash total kalau `db.transaction()` gagal connect (bukan cuma gagal publish) — ditambah try/catch pembungkus; (3) `tsx` tidak resolve `tsconfig.json` per-file untuk import lintas-paket lewat symlink `node_modules` (`@wadar/platform` dari `apps/worker`) — diperbaiki dengan `--tsconfig ../../tsconfig.json` eksplisit di skrip `dev`. Juga ditambah `tsup` (bundling `noExternal: [/^@wadar\//]`) untuk `apps/api`/`apps/worker` karena `node dist/main.js` butuh workspace package ter-bundle, bukan sekadar ter-typecheck.
